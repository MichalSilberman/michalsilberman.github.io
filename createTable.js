...
const createTable = () => {
    ...
    Object.entries(girlsData).map(([classNumber, studentsArray]) => {
        studentsArray.forEach(student => {
            const trForThisStudent = document.createElement('tr')
            trForThisStudent.setAttribute('id', student.phone);
...
const getRandomStudent = () => {
    //rearrange the students object to array of students
    const flattenedGirls = Object.entries(girlsData).reduce((accumulator, [className, students]) => {
        const classArray = students.map(student => ({ class: className, ...student }));
        return accumulator.concat(classArray);
    }, []);
    const theSelectedGirl = flattenedGirls[Math.floor(Math.random() * (26)) + 1]
    
    const containerDiv = document.createElement('div')
    containerDiv.innerText = Object.entries(theSelectedGirl).map(([key, value]) => `${key}: ${value}`).join(', \n');
    document.querySelector('#lottery').appendChild(containerDiv)
    document.getElementById(theSelectedGirl.phone).setAttribute('class', 'selectedRow')
